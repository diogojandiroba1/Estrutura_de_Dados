...
void split_child(node *x, int i, int ordem) {
    ...
    int median_idx = (ordem - 1) / 2;
    int z_key_count = 0;
    for (int j = median_idx + 1; j < ordem - 1; j++) {
        z->chaves[z_key_count] = y->chaves[j];
        z_key_count++;
    }
    z->qtd_chaves_atual = z_key_count;
    if (!y->is_folha) {
        int z_child_count = 0;
        for (int j = median_idx + 1; j < ordem; j++) {
            z->ArrayPonteiros[z_child_count] = y->ArrayPonteiros[j];
            z_child_count++;
        }
    }
    y->qtd_chaves_atual = median_idx;
    ...
    x->chaves[i] = y->chaves[median_idx];
    x->qtd_chaves_atual++;
}
